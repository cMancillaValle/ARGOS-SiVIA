...
def _migrar_sesiones(conn: sqlite3.Connection):
    """
    Si la tabla 'sesiones' ya existía antes de este cambio, le faltarán
    las columnas modulo_actual / ultima_actividad. Las agrega sin
    perder los datos existentes.
    """
    columnas = {row['name'] for row in conn.execute('PRAGMA table_info(sesiones)').fetchall()}
    ...
    if 'ultima_actividad' not in columnas:
        ...
        # Backfill para filas existentes (ALTER TABLE con DEFAULT dinámico no aplica retroactivo)
